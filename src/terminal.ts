...
function filterEnv(): Record<string, string> {
  ...
  for (const [key, value] of Object.entries(process.env)) {
    ...
    if (FILTERED_ENV_PREFIXES.some(prefix => key.startsWith(prefix))) continue;
    env[key] = value;
  }
  return env;
}
...
export class PtyManager {
  ...
  private startFlushTimer(): void {
    ...
    this.flushTimer = setInterval(() => this.flushOutputBuffers(), FLUSH_INTERVAL_MS);
  }
  ...
  create(
    ...
    const managed: ManagedPty = { id, projectName, terminalName, cwd, process: proc };
  ...
  openProject(projectName: string, cwd: string, terminals: TerminalConfig[]): string[] {
    ...
    try {
      ...
      return terminals.map(t => this.create(projectName, t.name, cwd, t.commands));
    } finally {
  ...
  split(existingPtyId: string): string | null {
    ...
    return this.create(existing.projectName, `split-${Date.now()}`, existing.cwd, []);
  }
  ...
  killAll(): void {
    for (const managed of this.ptys.values()) {
      try { managed.process.kill(); } catch { /* ignore on shutdown */ }
    }
    this.ptys.clear();
  ...
  getSessionInfo(id: string): { projectName: string; terminalName: string } | null {
    const m = this.ptys.get(id);
    return m ? { projectName: m.projectName, terminalName: m.terminalName } : null;
  }
}
